...
from lib.vis_utils.image import vis_image_mask_cv2
...
class GeoReF_Evaluator(DatasetEvaluator):
    """no use"""

    def __init__(self, cfg, dataset_name, distributed, output_dir, train_objs=None):
        self.cfg = cfg
        self.n_iter_test = cfg.MODEL.GeoReF.N_ITER_TEST
...
def georef_inference_on_dataset(cfg, model, data_loader, evaluator, amp_test=False):
    ...
    n_iter_test = cfg.MODEL.GeoReF.N_ITER_TEST
...
def georef_save_result_of_dataset(cfg, model, data_loader, output_dir, dataset_name, train_objs=None, amp_test=False):
    """
