...
def get_out_coor(cfg, coor_x, coor_y, coor_z):
    if (coor_x.shape[1] == 1) and (coor_y.shape[1] == 1) and (coor_z.shape[1] == 1):
        ...
        coor_[coor_ == cfg.MODEL.GeoReF.XYZ_HEAD.XYZ_BIN] = 0
        # normalize the coordinates to [0, 1]
        coor_ = coor_ / float(cfg.MODEL.GeoReF.XYZ_HEAD.XYZ_BIN - 1)
...
def get_out_mask(cfg, pred_mask):
    # (b,c,h,w)
    # output: (b, 1, h, w)
    mask_loss_type = cfg.MODEL.GeoReF.MASK_HEAD.MASK_LOSS_TYPE
...
def get_input_dim(cfg):
    backbone_cfg = cfg.MODEL.GeoReF.BACKBONE
