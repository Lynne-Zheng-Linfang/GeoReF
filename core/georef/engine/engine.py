...
from core.georef.datasets.data_loader import build_georef_train_loader, build_georef_test_loader
from core.georef.engine.engine_utils import vis_batch
from core.georef.engine.batching import batch_data, batch_updater
from .georef_evaluator import georef_inference_on_dataset, georef_save_result_of_dataset
from .georef_custom_evaluator import GeoReF_EvaluatorCustom
...
def get_evaluator(cfg, dataset_name, output_folder=None):
    ...
    if evaluator_type == "nocs":
        georef_eval_cls = GeoReF_EvaluatorCustom
        return georef_eval_cls(
            cfg,
            dataset_name,
...
def do_save_results(cfg, model, epoch=None, iteration=None):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        data_loader = build_georef_test_loader(cfg, dataset_name, train_objs=train_obj_names)
        georef_save_result_of_dataset(
            cfg,
            model,
            data_loader,
...
def do_test(cfg, model, epoch=None, iteration=None):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        data_loader = build_georef_test_loader(cfg, dataset_name, train_objs=evaluator.train_objs)
        results_i = georef_inference_on_dataset(cfg, model, data_loader, evaluator, amp_test=cfg.TEST.AMP_TEST)
...
def do_train(cfg, args, model, optimizer, resume=False):
    ...
    data_loader = build_georef_train_loader(cfg, train_dset_names)
    ...
    if train_2_ratio > 0.0 and len(train_2_dset_names) > 0:
        data_loader_2 = build_georef_train_loader(cfg, train_2_dset_names)
    ...
    with EventStorage(start_iter) as storage:
        ...
        for iteration in range(start_iter, max_iter):
            ...
            epoch_changed = True if (last_epoch != epoch) else False
            if ((epoch-1) >= cfg.TEST.EVAL_PERIOD) and (cfg.TEST.EVAL_PERIOD > 0) and epoch_changed and ((epoch-1) % cfg.TEST.EVAL_PERIOD == 0) and (iteration != max_iter - 1) and (iteration > start_iter):
                do_test(cfg, model, epoch=epoch-1, iteration=iteration)
