...
import cv2
from core.georef.models.GeoReF import build_model_optimizer
...
from core.georef.datasets.dataset_factory import register_datasets_in_cfg
from core.georef.engine.engine import do_test, do_train, do_save_results
...
def setup(args):
    ...
    cfg.SEED = int(time.time()%(2**32)) if (args.seed == -1) else args.seed
    print("*************** SEED", cfg.SEED)
    cfg.OUTPUT_DIR = cfg.OUTPUT_DIR+f'_{cfg.SEED}'
...
def main(args):
    ...
    logger.info(f"Used GeoReF module.")
    ...
    if args.eval_only:  # eval only --------------------------------------------------
        # checkpoint = MyCheckpointer(model, save_dir=cfg.OUTPUT_DIR)
        # checkpoint.resume_or_load(cfg.MODEL.WEIGHTS, resume=args.resume)
        # checkpoint.save("new_model")
        MyCheckpointer(model, save_dir=cfg.OUTPUT_DIR).resume_or_load(cfg.MODEL.WEIGHTS, resume=args.resume)
