...
from lib.structures import MyBitMasks, Poses, MyList
from core.utils.data_utils import read_image_mmcv
from core.utils.dataset_utils import (
    filter_empty_dets,
    filter_invalid_in_dataset_dicts,
    flat_dataset_dicts,
    load_georef_init_into_dataset,
...
from core.utils.depth_aug import add_noise_depth, random_fill_numbers_to_zero_points, randomly_drop_values
...
# from lib.lynne_lib.vision_utils import show_point_cloud

logger = logging.getLogger(__name__)


def build_georef_augmentation(cfg, is_train):
...
class GeoReF_DatasetFromList(Base_DatasetFromList):
    ...
    def __init__(self, cfg, split, lst: list, copy: bool = True, serialize: bool = True, flatten=False):
        ...
        """
        # cfg.INPUT = {'FORMAT': 'BGR', 'MIN_SIZE_TRAIN': (480,), 'MAX_SIZE_TRAIN': 640, 'MIN_SIZE_TRAIN_SAMPLING': 'choice', 'MIN_SIZE_TEST': 480, 'MAX_SIZE_TEST': 640, 'WITH_DEPTH': True, 'BP_DEPTH': False, 'AUG_DEPTH': True, 'NORM_DEPTH': False, 'DROP_DEPTH_RATIO': 0.2, 'DROP_DEPTH_PROB': 0.5, 'ADD_NOISE_DEPTH_LEVEL': 0.01, 'ADD_NOISE_DEPTH_PROB': 0.9, 'COLOR_AUG_PROB': 0.0, 'COLOR_AUG_TYPE': 'ROI10D', 'COLOR_AUG_CODE': '', 'COLOR_AUG_SYN_ONLY': False, 'RANDOM_FLIP': 'none', 'WITH_BG_DEPTH': False, 'BG_DEPTH_FACTOR': 10000.0, 'BG_TYPE': 'VOC_table', 'BG_IMGS_ROOT': 'datasets/VOCdevkit/VOC2012/', 'NUM_BG_IMGS': 10000, 'CHANGE_BG_PROB': 0.0, 'TRUNCATE_FG': False, 'BG_KEEP_ASPECT_RATIO': True, 'DZI_TYPE': 'uniform', 'DZI_PAD_SCALE': 1.0, 'DZI_SCALE_RATIO': 0.25, 'DZI_SHIFT_RATIO': 0.25, 'SMOOTH_XYZ': False, 'ALIGN_PCL': False, 'WITH_IMG': False, 'PCL_WITH_COLOR': False, 'SAMPLE_DEPTH_FROM_BALL': True, 'DEPTH_SAMPLE_BALL_RATIO': 0.6, 'FPS_SAMPLE': False, 'MAX_SYM_DISC_STEP': 0.01, 'BBOX_TYPE_TEST': 'est', 'INIT_POSE_TYPE_TRAIN': ['gt_noise'], 'INIT_SCALE_TYPE_TRAIN': ['gt_noise'], 'INIT_POSE_TRAIN_PATH': 'datasets/NOCS/train_init_poses/init_with_last_frame.pkl', 'INIT_POSE_TYPE_TEST': 'est', 'NOISE_ROT_STD_TRAIN': (10, 5, 2.5, 1.25), 'NOISE_ROT_STD_TEST': 15, 'NOISE_ROT_MAX_TRAIN': 45, 'NOISE_ROT_MAX_TEST': 45, 'NOISE_TRANS_STD_TRAIN': [(0.02, 0.02, 0.02), (0.01, 0.01, 0.01), (0.005, 0.005, 0.005)], 'NOISE_TRANS_STD_TEST': [(0.01, 0.01, 0.005), (0.01, 0.01, 0.01), (0.005, 0.005, 0.01)], 'INIT_TRANS_MIN_Z': 0.1, 'NOISE_SCALE_STD_TRAIN': [(0.01, 0.01, 0.01), (0.005, 0.005, 0.005), (0.002, 0.002, 0.002)], 'NOISE_SCALE_STD_TEST': [(0.001, 0.005, 0.001), (0.005, 0.001, 0.005), (0.01, 0.01, 0.01)], 'INIT_SCALE_MIN': 0.04, 'RANDOM_TRANS_MIN': [-0.35, -0.35, 0.5], 'RANDOM_TRANS_MAX': [0.35, 0.35, 1.3], 'RANDOM_SCALE_MIN': [0.04, 0.04, 0.04], 'RANDOM_SCALE_MAX': [0.5, 0.3, 0.4], 'MEAN_MODEL_PATH': 'datasets/NOCS/obj_models/cr_normed_mean_model_points_spd.pkl', 'KPS_TYPE': 'mean_shape', 'USE_CMRA_MODEL': True, 'WITH_NEG_AXIS': False, 'BBOX3D_AUG_PROB': 0.5, 'RT_AUG_PROB': 0.5, 'DEPTH_BILATERAL_FILTER_TEST': False, 'DEPTH_HOLE_FILL_TEST': False, 'NUM_KPS': 1024, 'NUM_PCL': 1024, 'ZERO_CENTER_INPUT': True, 'NOCS_DIST_THER': 0.03, 'CANONICAL_ROT': [(1, 0, 0, 0.5), (0, 0, 1, -0.7)], 'CANONICAL_TRANS': [0, 0, 1.0], 'CANONICAL_SIZE': [0.2, 0.2, 0.2], 'OCCLUDE_MASK_TEST': False, 'WITH_PCL': True}

        # ResizeShortestEdge
        self.augmentation = build_georef_augmentation(cfg, is_train=(split == "train"))
        ...
        if cfg.INPUT.COLOR_AUG_PROB > 0 and cfg.INPUT.COLOR_AUG_TYPE.lower() == "ssd":
            ...
            logging.getLogger(__name__).info("Color augmentation used in training: " + str(self.augmentation[-1])) # False

        # fmt: off
        self.img_format = cfg.INPUT.FORMAT  # BGR

        self.with_depth = cfg.INPUT.WITH_DEPTH # True
        self.aug_depth = cfg.INPUT.AUG_DEPTH # True
        self.drop_depth_ratio = cfg.INPUT.DROP_DEPTH_RATIO # 0.2
        self.drop_depth_prob = cfg.INPUT.DROP_DEPTH_PROB # 0.5
        self.add_noise_depth_level = cfg.INPUT.ADD_NOISE_DEPTH_LEVEL # 0.01
        self.add_noise_depth_prob = cfg.INPUT.ADD_NOISE_DEPTH_PROB # 0.9
        self.with_bg_depth = cfg.INPUT.WITH_BG_DEPTH # False

        # NOTE: color augmentation config
        self.color_aug_prob = cfg.INPUT.COLOR_AUG_PROB # 0.0
        self.color_aug_type = cfg.INPUT.COLOR_AUG_TYPE # 'ROI10D'
        ...
        self.model_points = {}
        # -------------------------------------------------
        self.with_last_poses = "last_frame" in cfg.INPUT.INIT_POSE_TYPE_TRAIN # False
    ...
    def read_data_train(self, dataset_dict):
        ...
        # dict_keys(['dataset_name', 'file_name', 'height', 'width', 'scene_im_id', 'cam', 'img_type', 'depth_file', 'coord_file', 'annotations'])
        ...
        K = self._transfer_K_to_tensor(dataset_dict)

        # load mug meta information =============================================
        self._get_mug_meta(dataset_name)
        self._get_mean_scale(dataset_name)

        coord = self._load_nocs_coordinate(dataset_dict)

        if self.with_depth:
            depth, in_depth_channels = self._load_depth_image(dataset_dict, K)
        else:
            in_depth_channels = 0

        img_type = dataset_dict.get("img_type", "real")  # real/camera
        do_replace_bg = self._need_replace_bg(img_type) # False

        if do_replace_bg: #False
            image, bg_depth, fg_mask = self._replace_bg(image, dataset_dict, cfg, im_H_ori, im_W_ori, in_depth_channels)

        if self._need_aug_color(): # False
            image = self._aug_color(image, img_type)
        ...
        if input_cfg.WITH_IMG: # False
            K = self._scale_camera_intrinsic(im_H, im_H_ori, im_W, im_W_ori, dataset_dict)
        ...
        if self.with_depth:
            if do_replace_bg and self.with_bg_depth: # False
                depth = self._replace_depth_bg(depth, fg_mask, bg_depth)

            depth = self._aug_depth_image(depth, im_W, im_W_ori, im_H, im_H_ori, in_depth_channels)
        ...
        if "annotations" in dataset_dict:
            instances = self._transform_annotations(dataset_name, transforms, image_shape, 
                                                    dataset_dict, depth, K, image, coord)
            dataset_dict["instances"] = filter_empty_instances(instances, by_box=False, by_mask=True)

        return dataset_dict
    
    def _add_sym_infos_and_mean_points_and_mean_scales_to_instances(self, dataset_dict, instances, dataset_name, add_last_frame_poses = False, add_fps_points=False, ):
        ...
        for obj_id, mug_handle, inst_name in zip(
            ...
            if "cmra" in dataset_name and self.cfg.INPUT.USE_CMRA_MODEL:
                ref_key = inst_name
            else:
                ref_key = obj_name
            obj_mean_points.append(self.mean_model_dict[ref_key])
            if add_last_frame_poses:
    ...
    def _transform_annotations(self, dataset_name, transforms, image_shape,
                               dataset_dict, depth, K, image, coord, ):
        ...
        instances = annotations_to_instances(self.cfg, annos, image_shape)

        rgb_list, pcl_list, nocs_list = self._sample_rgb_pcl_and_nocs(instances, depth, K, image, coord, dataset_name)
        ...
        instances = self._add_sym_infos_and_mean_points_and_mean_scales_to_instances(dataset_dict, instances,
                                                                                     dataset_name, 
                                                                                     add_last_frame_poses = self.with_last_poses, add_fps_points=self.cfg.INPUT.KPS_TYPE.lower() == "fps" )
        instances.remove("inst_names")
        return instances
    
    def _sample_rgb_pcl_and_nocs(self, instances, depth, K, image, coord, dataset_name, ):
        dset_meta = MetadataCatalog.get(dataset_name)
        pcl_list = []
        nocs_list = []
        rgb_list = []
        _trunc_fg = self.cfg.INPUT.get("TRUNCATE_FG", False)
        if _trunc_fg:
        ...
        for mask, pose, scale, cat_id, inst_name in zip(masks, poses, scales, cat_ids, inst_names):
            if self.cfg.INPUT.SAMPLE_DEPTH_FROM_BALL:
                rgb, pcl, nocs = crop_ball_from_depth_image(
                    ...
                    scale,
                    coord=coord,
                    ratio= self.cfg.INPUT.DEPTH_SAMPLE_BALL_RATIO,
                    cam_intrinsics=torch.from_numpy(K),
                    num_points= self.cfg.INPUT.NUM_PCL,
                    device="cpu",
                    fps_sample= self.cfg.INPUT.FPS_SAMPLE,
                )
            else:  # sample randomly in mask
                rgb, pcl, nocs = crop_mask_depth_image(
                    image_th, depth_bp, mask, coord=coord, num_points=self.cfg.INPUT.NUM_PCL
                )
        ...
        return rgb_list, pcl_list, nocs_list

    def _replace_depth_bg(self, depth, fg_mask, bg_depth):
        mask_bg_depth = (fg_mask == 0).astype(np.bool)  # must be bool
        depth[mask_bg_depth] = bg_depth[mask_bg_depth]
        return depth

    def _scale_camera_intrinsic(self, im_H, im_H_ori, im_W, im_W_ori, dataset_dict):
    ...
    def _need_aug_color(self):
        return self.cfg.INPUT.WITH_IMG and self.color_aug_prob > 0 and self.color_augmentor is not None

    def _aug_color(self, image, img_type):
        # cfg.INPUT.WITH_IMG = False
        if np.random.rand() < self.color_aug_prob:
            if self.cfg.INPUT.COLOR_AUG_SYN_ONLY and img_type not in ["real"]:
                image = self._color_aug(image, self.color_aug_type)
            else:
                image = self._color_aug(image, self.color_aug_type)
        return image

    def _load_mug_meta_information(self, dataset_name):
        # load mug meta information =============================================
        self._get_mug_meta(dataset_name)
        self._get_mean_scale(dataset_name)
        return

    def _need_replace_bg(self, img_type):
        ...
        if img_type == "syn":
            ...
            if self.cfg.INPUT.WITH_IMG and np.random.rand() < self.cfg.INPUT.CHANGE_BG_PROB:
                log_first_n(logging.WARNING, f"replace bg for {img_type}", n=10)
                do_replace_bg = True
        return do_replace_bg
    
    def _load_nocs_coordinate(self, dataset_dict):
    ...
    def _transfer_K_to_tensor(self, dataset_dict):
    ...
    def _load_depth_image(self, dataset_dict, K):
        ...
        if self.cfg.INPUT.BP_DEPTH: # False
            in_depth_channels = 3
        ...
        return depth, in_depth_channels
    
    def _replace_bg(self, image, dataset_dict, cfg, im_H_ori, im_W_ori, in_depth_channels):
        ...
        if self.with_depth and self.with_bg_depth:
            ...
            bg_depth = None
        return image, bg_depth, fg_mask
    
    def _aug_depth_image(self, depth, im_W, im_W_ori, 
                         im_H, im_H_ori, in_depth_channels):

        if self.aug_depth:  # randomly fill 0 points
            depth = random_fill_numbers_to_zero_points(depth)

        if self.aug_depth and np.random.rand(1) < self.drop_depth_prob:  # drop 20% of depth values
            depth = randomly_drop_values(depth, self.drop_depth_ratio)

        # add gaussian noise
        if self.aug_depth and np.random.rand(1) < self.add_noise_depth_prob:
            # # add gaussian noise to >0 regions
            depth = add_noise_depth(depth, level=self.add_noise_depth_level) # add 0.01 level
...
def build_georef_train_loader(cfg, dataset_names):
    ...
    dataset_dicts = get_detection_dataset_dicts(
        ...
        min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE if cfg.MODEL.KEYPOINT_ON else 0, # min_keypoints = 0
        proposal_files=cfg.DATASETS.PROPOSAL_FILES_TRAIN if cfg.MODEL.LOAD_PROPOSALS else None, # proposal_files = None
    )

    dataset_dicts = filter_invalid_in_dataset_dicts(dataset_dicts, visib_thr=cfg.DATALOADER.FILTER_VISIB_THR)

    dataset = GeoReF_DatasetFromList(cfg, split="train", lst=dataset_dicts, copy=False, flatten=False)

    sampler_name = cfg.DATALOADER.SAMPLER_TRAIN # 'TrainingSampler'
    logger = logging.getLogger(__name__)
    logger.info("Using training sampler {}".format(sampler_name))
    if sampler_name == "TrainingSampler": # This one
...
def build_georef_test_loader(cfg, dataset_name, train_objs=None):
    ...
    dataset = GeoReF_DatasetFromList(cfg, split="test", lst=dataset_dicts, flatten=False)
