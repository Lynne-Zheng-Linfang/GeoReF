...
sys.path.append('../lib')
from spd_align import align_nocs_to_depth
from spd_utils import load_depth
    ...
    for subset in ['real_train', 'real_test']:
        ...
        for folder in folder_list:
            ...
            for img_full_path in img_paths:
                ...
                img_list.append(img_path)
        with open(os.path.join(data_dir, 'REAL', subset.split('_')[-1]+'_list_all.txt'), 'w') as f:
            for img_path in img_list:
                f.write("%s\n" % img_path)
...
def annotate_test_data(data_dir):
    ...
    for source, img_list, intrinsics, subset in subset_meta:
        ...
        for img_path in tqdm(img_list):
            ...
            nocs_dir = os.path.join(data_dir, 'spd_results/nocs_results')
            if source == 'CAMERA':
